...
namespace wxl::scripts::modernm2
{
    ...
    void ModernM2::OnModelLoadPre(const ev::ModelLoadArgs& a)
    {
        ...
        {
            if (md->magic == fmt::kMagicMD21)
                WLOG_WARN("modern-m2: raw MD21 reached runtime for %s; host dechunk/SKID transform did not run",
                          m2::PathStem(a.model));
            return;
        }
        ...
        const uint32_t workSize = downport::WorkSize(buf, size, pathStem ? pathStem : "");
        void* image = buf;
        if (workSize == size)
        {
            if (!downport::ProcessInPlace(buf, size, size, pathStem ? pathStem : "")) return;
        }
        else
        {
            ...
            if (!downport::ProcessInPlace(out, size, workSize, pathStem ? pathStem : "")) { m2::FreeBuffer(out); return; }
    ...
    void ModernM2::OnSkinFinalize(const ev::M2SkinFinalizeArgs& a)
    {
        ...
        if (md && sk)
            skin::Rebuild(md, sk, m2::PathStem(a.model));
    }
