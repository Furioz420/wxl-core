...
namespace wxl::scripts::modernm2::md21
{
    ...
    // FileDataID/path -> sidecar bytes. The host can read archives here; the runtime fallback cannot.
    using SidecarReader = bool (*)(uint32_t fileDataId, const std::string& path, std::vector<uint8_t>& outBytes);

    // Reports whether `in` is an MD21 chunked container.
    bool IsMd21(std::span<const uint8_t> in);

    // Extract the MD20 block from an MD21 container and inline TXID-referenced texture names. `resolve`
    // maps FileDataIDs to paths. Returns false when `in` is not an MD21 container or is malformed.
    bool Dechunk(std::span<const uint8_t> in, TxidResolver resolve, std::vector<uint8_t>& out);

    // Host form: also inlines SKID-referenced skeleton sidecars when the source moved sequences/bones out
    // of the MD20 body.
