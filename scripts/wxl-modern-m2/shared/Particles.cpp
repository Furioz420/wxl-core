...
namespace wxl::scripts::modernm2::particles
{
    ...
    namespace
    {
        ...
        // textureId @+0x16: the client reads a flat u16 index into header.textures. The modern multi-texture
        // form (flag 0x10000000) packs three 5-bit ids, so keep the first and park the rest.
        constexpr uint32_t kTextureIdOff       = 0x16;
        constexpr uint32_t kFlagMultiTex       = 0x10000000;
        constexpr uint32_t kMultiTexBlockSize  = 0x10;
        constexpr uint32_t kInvalidParticleTag = 0xFF;
    ...
    void Compact(fmt::M2Header* md, uint32_t fileSize)
    {
        ...
        if (md->particleEmitters.offset >= fileSize) return;

        const uint32_t maxBySource = (fileSize - md->particleEmitters.offset) / kStrideSource;
        const uint32_t sourceCount = std::min(md->particleEmitters.count, maxBySource);
        if (!sourceCount) return;

        std::vector<uint16_t> multiTexTags(sourceCount, 0);
        uint32_t compactedCount = sourceCount;
        for (uint32_t i = 0; i < sourceCount; ++i)
        {
            uint8_t* s = arr + i * kStrideSource;
            uint8_t* d = arr + i * kStrideClient;

            const uint8_t* multi = s + kStrideClient;
            if (static_cast<size_t>(multi - md->base()) + kMultiTexBlockSize <= fileSize)
                multiTexTags[i] = Rd16(multi + 2);
        ...
        while (compactedCount > 0 && multiTexTags[compactedCount - 1] == kInvalidParticleTag)
            --compactedCount;
        if (compactedCount != md->particleEmitters.count)
        {
            WLOG_INFO("modern-m2: compacted particles count %u -> %u", md->particleEmitters.count, compactedCount);
            md->particleEmitters.count = compactedCount;
        }
    }
