...
namespace wxl::scripts::modernm2::textures
{
    ...
    namespace
    {
        ...
        uint32_t BuildRepairedLoops(const fmt::M2Header* md, uint32_t fileSize, std::vector<uint32_t>& loops)
        {
            ...
            for (uint32_t i = 0; i < md->textureTransforms.count; ++i)
            {
                ...
                for (uint32_t t = 0; t < 3; ++t)
                {
                    ...
                    if (DesiredTrackLoop(md, fileSize, track, loops, loop))
                        ++repaired;
                }
            }
                                                  ...
                                                  uint32_t appendOffset, std::string_view name)
    {
        ...
        const uint32_t repaired = BuildRepairedLoops(md, fileSize, loops);
                                         ...
                                         std::string_view name)
    {
        ...
        for (uint32_t i = 0; i < md->textureTransforms.count; ++i)
        {
                ...
                for (uint32_t t = 0; t < 3; ++t)
                {
                    ...
                    const uint32_t duration = TrackDurationMs(md, fileSize, track);
                    uint16_t loop = kNoGlobalSequence;
                    if (!DesiredTrackLoop(md, fileSize, track, loops, loop)) continue;
                    if (DebugTextureLoops(name))
                    {
