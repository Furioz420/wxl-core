...
namespace wxl::runtime::lua
{
    ...
    void InstallHooks()
    {
        ...
        wxl::hook::Install(
            ...
            &ValidateFunctionPointer, &g_origValidateFunctionPointer);
    }
...
namespace
{
    // Normal phase: create the callback-validator detour so the global EnableAll arms it.
    bool InstallLuaBindings()
    {
        wxl::runtime::lua::InstallHooks();
        return true;
    }

    // Post-enable phase: the validator detour is only live after EnableAll, so mark it active and push
    // the addon-facing globals into the current FrameScript state only now -- registering them earlier
    // would let the client's native pointer validator reject WXL's callbacks.
    bool ActivateLuaBindings()
    {
...
WXL_REGISTER_FEATURE("luabindings", wxl::features::kLuaBindings, InstallLuaBindings)
WXL_REGISTER_FEATURE_PHASED("luabindings-activate", wxl::features::kLuaBindings, ActivateLuaBindings,
                            ::wxl::hook::Phase::PostEnable)
