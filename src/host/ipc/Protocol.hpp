...
// isolated host session (mailbox, events, blob sections, and singleton mutex).
namespace wxl::ipc
{
    constexpr uint32_t kMinChannels = 2;
    constexpr uint32_t kMaxChannels = 16;

    // The first %u always identifies the owning Wow.exe process.
...
#pragma pack(pop)
    ...
    // Format the per-session / per-channel names into the caller's buffer (>= 64 bytes).
